...
def diversification_grasp(matrice, pattern):
    
    modif = False
    best_rank, best_val_sing, _ = fobj(matrice, pattern)
    m, n = M.shape
    ...
    while num_candidats>0:
        ...
        if col_tuple not in candidats_colonnes_set:
            ...
            for index, col in enumerate(col_modif):
                pattern_copy[:, col] = col_candidat[index]
            rang, val_sing, _ = fobj(matrice, pattern_copy)

            if (rang < best_rank):
                best_rank = rang
                print(f"YOUPIIII Colonne {col+1}/{n} :  rang = {rang}")
                best_pattern = pattern_copy.copy()
                grasp_data.append((col, rang, val_sing))  # Collecte des métriques
                modif = True
            
            elif (rang == best_rank) and val_sing<best_val_sing:
                best_pattern = pattern_copy.copy()
                best_val_sing = val_sing
                modif = True
    
    return best_pattern, best_rank, modif
...
if __name__ == "__main__":
    ...
    final_pattern = best_pattern.copy()
    final_rank = rang
